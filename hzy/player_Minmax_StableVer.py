...
class Player:
    def __init__(self, isFirst, array):
        ...
        self.array = array
        self.maxValue = 2E9
        self.weight = [1.0, 2.0, 5.0, 11.0, 23.0, 47.0, 95.0, 191.0, 383.0, 767.0, 1535.0, 3071.0, 6143.0]  # t
    ...
    def scoreMove(self, board):
        myScoreLst = board.getScore(self.isFirst)
        RivalScoreLst = board.getScore(not self.isFirst)
        total = self.weightSum(myScoreLst) - self.weightSum(RivalScoreLst)  # 更有进攻性
        # move的情况空格有额外两分
    ...
    def minmaxDecisionF(self, board, currentRound, mode):  # 针对先手进攻局
        ...
        if mode == 'direction':
            ...
            if len(actions) > 4 and (next_pos is not None):  # t
                return next_pos
            else:
                depth = self.getDepth(currentRound, board)
