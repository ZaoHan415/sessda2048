...
class Player:
    ...
    def _minMaxRecur(self, board, depth, phase, currentRound, inc, alpha=-(2E9), beta=+(2E9)):
        ...
        if len(board.getNone(self.isFirst))<10 or len(board.getNone(not self.isFirst))<10:
            ...
            if phase==2 or phase==3:
                ...
                newBoard = board.copy()
                newBoard.add(peer, board.getNext(peer, currentRound))
                if phase==0:
                    newBoard.add(False, board.getNext(False, currentRound))
                curScore = self._minMaxRecur(newBoard, 2, 2, currentRound, inc, alpha, beta)
                return curScore
            return alpha if flag else beta    
