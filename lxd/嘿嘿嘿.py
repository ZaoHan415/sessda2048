...
class Player:
    def __init__(self, isFirst, array):
        ...
        self.weight = [float(4**i) for i in range(13)]
    ...
    def getActions(currentRound, board, mode, peer):
        if mode == 'position':
            ...
            return [3, 0, 1, 2] if peer else [2, 0, 1, 3]
    ...
    def score(self, board):  # 最简单的局面估值函数
        ...
        total = self.weightSum(myScoreLst) - self.weightSum(RivalScoreLst)*32
    ...
    def _minMaxRecurF(self, board, depth, phase, currentRound, inc, alpha=-(2E9), beta=+(2E9)):
        ...
        if phase == 2 or phase == 3:
            if phase == 3:
                currentRound += 1
            act= [3, 0, 1, 2] if peer else [2, 0, 1, 3]
            for d in act:
                newBoard = board.copy()
            ...
            if len(board.getNext(peer, currentRound)) == 0:
                ...
                if not flag and (curScore < beta):
                    ...
                    if alpha >= beta:
                        return beta
            #elif posLstLen <= 6 and currentRound > 0.3*self.maxRounds:
            # elif posLstLen <= 5:
                #if inc < 4:
                #    depth += 1
                #    inc += 1
        ...
        if phase == 2 or phase == 3:
            ...
            if len(board.getNext(peer, currentRound)) == 0:
                ...
                if not flag and (curScore < beta):
                    ...
                    if alpha >= beta:
                        return beta
            #elif posLstLen <= 6 and currentRound > 0.2*self.maxRounds:
            # elif posLstLen <= 5:
                #if inc < 4:
                #    depth += 1
                #    inc += 1
    ...
    def minmaxDecisionF(self, board, currentRound, mode):  # 针对先手进攻局
        ...
        if mode == 'direction':
            # phase 0 1 2 3
            phase = 2
            depth = 9 if currentRound<300 else 13
            for d in actions:
                newBoard = board.copy()
    ...
    def minmaxDecisionNF(self, board, currentRound, mode):
        ...
        if mode == 'direction':
            # phase 0 1 2 3
            phase = 3
            depth = 8 if currentRound<300 else 12
            for d in actions:
                newBoard = board.copy()
                if newBoard.move(self.isFirst, d):
